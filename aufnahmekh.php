<?php
	...
	*/
	 ...
	 *
	 * @param boolean $_GET["nocache"] can be empty, e.g. ?nocache&date=xxx
	 */
	
	if(isset($_GET["date"])){
		$getbydate = strtotime($_GET["date"]);
		/**
		 * If the date can't be converted to a timestampt, the script throws an error message and exits with http error 400.
		 */
		if(!$getbydate){
			http_response_code(400);
			exit($_GET["date"]." seems to be not a valid date.<br>The date should be formatted like this 2019-09-28 or 28.09.2019.");
		}
	}
	 ...
	 */
	if(!file_exists("aufnahmekh.json") || date("d.m.Y", filemtime("aufnahmekh.json")) != date("d.m.Y") || isset($_GET["nocache"])){
